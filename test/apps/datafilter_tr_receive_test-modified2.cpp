...
struct DatafilterConfig {
    ...
    std::string get_connection_name(size_t app_id, size_t group_id,
                                    size_t conn_id) {
        std::stringstream ss;
        ss << tr_writer_conn_name << app_id << "_G" << group_id << "_C"
           << conn_id << "_";
        return ss.str();
    }
    std::string get_group_connection_name(size_t app_id, size_t group_id) {
        std::stringstream ss;
        ss << tr_writer_conn_name << app_id << "_G" << group_id << "_.*";
        return ss.str();
    }
    ...
    void configure_iomanager() {
        ...
        std::string tr_writer_conn = get_connection_name(0, 0, 0);
        connections.emplace_back(
            Connection{ConnectionId{tr_writer_conn, "TriggerRecord"}, conn_addr,
...
struct TRRewriter {
    ...
    struct TRWriterInfo {
        size_t conn_id;
        size_t group_id;
        bool is_group_subscriber = false;
    ...
    void get_adc_value() {
        // something here for get the fragment adc values out.
    }
    ...
    void receive_tr(size_t run_number1) {
        ...
        std::function<void(dunedaq::datafilter::Handshake)> str_receiver_cb =
            [&](dunedaq::datafilter::Handshake msg) {
                ...
                std::cout << "receiver callback: " << msg.msg_id << 'n';
            };
        ...
        std::for_each(
            ...
            [=, &last_received](std::shared_ptr<TRWriterInfo> info) {
                auto recv_proc = [=, &last_received](trigger_record_ptr_t& tr) {
                    ...
                    auto fragment_size =
                        tr->get_fragments_ref().at(0)->get_size();

                    int file_index = 0;

                    TLOG() << "run_number " << run_number
                           << ", trigger number: " << trigger_number;
                    TLOG() << "fragment size " << fragment_size;
                    info->msgs_received++;
                    last_received = std::chrono::steady_clock::now();

                    TLOG() << "num_messages " << config.num_messages
                           << "msgs_received " << info->msgs_received;
                    std::string app_name = "test";
                    std::string ofile_name =
                        config.odir + "/" + config.output_h5_filename +
                        std::to_string(trigger_number) + ".hdf5";

                    auto data = tr->get_fragments_ref().at(0)->get_data();
                    auto hdr_size =
                        sizeof(tr->get_fragments_ref().at(0)->get_header());
                    auto WIBEthFrame_size =
                        sizeof(dunedaq::fddetdataformats::WIBEthFrame);
                    TLOG() << "hdr_size " << hdr_size << " WIBEthFrame_size "
                           << WIBEthFrame_size;

                    int nframes = (fragment_size - hdr_size) / WIBEthFrame_size;

                    TLOG() << "nframes " << nframes;
                    for (auto i = 0; i < nframes; ++i) {
                        ...
                        for (auto j = 0; j < nsamples; ++j) {
                            v.emplace_back(std::vector<uint16_t>(nchannels));
                            for (auto k = 0; k < nchannels; ++k) {
                                auto adc_value = fr->get_adc(k, j);
                                v[j][k] = adc_value;
                                if (adc_value > 0) {
                                    TLOG() << " j " << j << " k " << k
                                           << " adc value " << adc_value;
                    ...
                    */
                    ...
                    TLOG() << "Output trigger records to " << ofile_name;
                    // create the file
                    std::unique_ptr<HDF5RawDataFile> h5file_ptr(
                        new HDF5RawDataFile(ofile_name, run_number, file_index,
                    ...
                    if (info->msgs_received == config.num_messages) {
                        TLOG_DEBUG(7) << "Complete condition reached, sending "
                                         "init message for "
        ...
        while (!all_done) {
            ...
            TLOG_DEBUG(9) << "Done: " << recvrs_done << ", expected: "
                          << config.num_groups *
