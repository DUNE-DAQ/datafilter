...
void print_usage(nlohmann::json fl_conf_in1, std::string ifile_name,
                 std::string hw_map_file_name, std::string ofile_name) {
    nlohmann::json fl_conf;

    // fl_conf =
    // fl_conf_in["file_layout"].get<hdf5filelayout::FileLayoutParams>();
    ...
    TLOG() << "Usage: HDF5LIBS_TestWriter <configuration_file> "
              "<hardware_map_file> <output_file_name>";
}

void data_writer2(std::string ofile_name, int run_number, int file_index,
                  std::string app_name, nlohmann::json fl_conf_in1,
    ...
    std::string input_h5_filename =
        ...
        "np04_coldbox_run014182_0000_dataflow0_20220712T102315.hdf5";
    // open h5 file
    ...
    //      //fl_conf =
    //      j_in["filelayout_params"].get<hdf5filelayout::FileLayoutParams>();
    //      fl_conf =
    //      j_in["path_param_list"].get<hdf5filelayout::FileLayoutParams>();
    //      //fl_conf1 =
    //      fl_conf_in1["path_param_list"].get<hdf5filelayout::FileLayoutParams>();
    //
    ////      run_number = fl_conf_in["run_number"];
    ////      file_index = fl_conf_in["file_index"].get<int>();
    //
    ////      trigger_count = fl_conf_in["trigger_count"].get<int>();
    ////      fragment_size = fl_conf_in["data_size"].get<int>() +
    ///sizeof(FragmentHeader); /      stype_to_use =
    ///SourceID::string_to_subsystem(fl_conf_in["subsystem_type"].get<std::string>());
    ////      dtype_to_use =
    ///DetID::string_to_subdetector(fl_conf_in["subdetector_type"].get<std::string>());
    ////      ftype_to_use =
    ///string_to_fragment_type(fl_conf_in["fragment_type"].get<std::string>());
    ...
    int trigger_count = 1;
    //    const int fragment_size = fl_conf_in["data_size"].get<int>() +
    //    sizeof(FragmentHeader); const SourceID::Subsystem stype_to_use =
    //    SourceID::string_to_subsystem(fl_conf_in["subsystem_type"].get<std::string>());
    //    const DetID::Subdetector dtype_to_use =
    //    DetID::string_to_subdetector(fl_conf_in["subdetector_type"].get<std::string>());
    //    const FragmentType ftype_to_use =
    //    string_to_fragment_type(fl_conf_in["fragment_type"].get<std::string>());
    ...
    //    const int fragment_size = j_in["data_size"].get<int>() +
    //    sizeof(FragmentHeader); const SourceID::Subsystem stype_to_use =
    //    SourceID::string_to_subsystem(j_in["subsystem_type"].get<std::string>());
    //    const GeoID::SystemType gtype_to_use =
    //    GeoID::string_to_system_type(j_in["fragment_type"].get<std::string>());
    //      const GeoID::SystemType gtype_to_use =
    //      GeoID::string_to_system_type("WIP");
    const FragmentType ftype_to_use =
        string_to_fragment_type(j_in["fragment_type"].get<std::string>());
    ...
    //           << "\nSubsystem: " <<
    //           SourceID::subsystem_to_string(stype_to_use)
    //           << "\nFragment size (bytes, incl. header): " << fragment_size;

    // create the HardwareMapService
    //    std::shared_ptr<dunedaq::detchannelmaps::HardwareMapService>
    //    hw_map_service(
    ...
    //                      .count();
    //
    //      TLOG() << "\tWriting trigger " << trig_num << " with time_stamp " <<
    //      ts;
    //
    ...
    //      //trh_data.element_id = SourceID(SourceID::Subsystem::kTRBuilder,
    //      0);
    //
    ...
    //
    //        auto frag_ptr = std::make_unique<Fragment>(dummy_data.data(),
    //        dummy_data.size());
    //        //auto frag_ptr = std::make_unique<Fragment>(adc_data.data(),
    //        adc_data.size()); frag_ptr->set_header_fields(fh);
    //
    ...
    //
    //   // TLOG() << "Finished writing to file " <<
    //   h5_raw_data_file.get_file_name();
    //   // TLOG() << "Recorded size: " << h5_raw_data_file.get_recorded_size();
    //
}

void
// data_writer(nlohmann::json fl_conf_in, std::string ifile_name, std::string
// hw_map_file_name, std::string ofile_name)
data_writer(nlohmann::json fl_conf_in, std::string ifile_name) {
    ...
    //    app_name= fl_conf_in['application_name'];
    //    fl_conf =
    //    fl_conf_in["filelayout_params"].get<hdf5filelayout::FileLayoutParams>();
    //    TLOG() << "Read 'file_layout' configuration:\n";
    //    TLOG() << fl_conf;
    //  } catch (...) {
    //    TLOG() << "ERROR: Improper 'file_layout' configuration in " <<
    //    ifile_name; return 1;
    //  }
    //
    //  // read test writer app configs
    ...
    //  SourceID::string_to_subsystem(j_in["subsystem_type"].get<std::string>());
    //  const DetID::Subdetector dtype_to_use =
    //  DetID::string_to_subdetector(j_in["subdetector_type"].get<std::string>());
    //  const FragmentType ftype_to_use =
    //  string_to_fragment_type(j_in["fragment_type"].get<std::string>()); const
    //  int element_count = j_in["element_count"].get<int>();
    //
    //  TLOG() << "\nOutput file: " << ofile_name << "\nRun number: " <<
    //  run_number << "\nFile index: " << file_index
    //         << "\nNumber of trigger records: " << trigger_count << "\nNumber
    //         of fragments: " << element_count
    ...
    //
    //      auto frag_ptr = std::make_unique<Fragment>(dummy_data.data(),
    //      dummy_data.size()); frag_ptr->set_header_fields(fh);
    //
    //      // add fragment to TriggerRecord
    //      tr.add_fragment(std::move(frag_ptr));
    //
    ...
    //
    //  TLOG() << "Finished writing to file " <<
    //  h5_raw_data_file.get_file_name(); TLOG() << "Recorded size: " <<
    //  h5_raw_data_file.get_recorded_size();
    //
    // return 0;
}
