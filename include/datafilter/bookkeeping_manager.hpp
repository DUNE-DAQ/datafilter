...
struct BookkeepingReceiver {
  ...
  void stop() {
    ...
    // (TRD assigned_to_filter, FRW file_completed, TRD re_recorded/write_failed)
    // are written even if stop() is called before FRW's write-wait completes.
    // 10-minute timeout covers any realistic FRW processing time.
    {
      std::unique_lock<std::mutex> lk(all_bk_mutex);
      bool got_final = all_bk_cv.wait_for(
          lk, std::chrono::minutes(10),
          [this] { return all_bk_received.load(std::memory_order_acquire); });
      if (!got_final)
        TLOG() << "stop(): timed out waiting for final TRD BK — stopping anyway";
      else
...
private:
  ...
  // Translates kFileCompleted → kReRecorded so TRD's existing status check passes.
  // Preserves from_id="FilterResultWriter" so TRD's from_id check passes unchanged.
  void send_completion_to_trd(const dunedaq::datafilter::BookKeeping &frw_bk) {
    ...
    if (!sender) {
      TLOG() << "send_completion_to_trd: failed to get sender for " << m_bk_trd_uid;
      return;
    }
    try {
      sender->send(std::move(fwd), std::chrono::milliseconds(2000));
      TLOG() << "BookkeepingReceiver: forwarded FRW completion ("
             << frw_bk.tr_status << " -> " << fwd.tr_status
             << ") to TRD via " << m_bk_trd_uid;
  ...
  void receive_bk() {
    ...
    auto str_receiver_cb = [&](dunedaq::datafilter::BookKeeping bk) {
      ...
      if (is_from_trdisp &&
        ...
        TLOG() << "New pipeline cycle detected — resetting per-cycle counters";
      }
      ...
      if (cnt == 1) {
        ...
        // send_bk() forwards initial BK to FRW if a BK output is configured.
        // Guard with try-catch so that a missing/unconfigured output does not
        // throw past this point and skip run_info.set() and queuing below.
        if (!m_bk_tx_uid.empty()) {
          try {
            send_bk(bk);
          } catch (const std::exception &e) {
      ...
      // When FRW signals file completion (kFileCompleted or kWriteFailed),
      // forward a translated confirmation to TRD via bookkeeping2.
      // DataFilter is the intermediary — FRW no longer sends directly to TRD.
      if (is_from_writer &&
      ...
      // Detect TRD's final BK (kReRecorded or kWriteFailed) — sent after TRD
      // receives the forwarded completion and calls WriteJSON.
  ...
  void write_to_file() {
    ...
    for (;;) {
      ...
      if (!have_file.load(std::memory_order_acquire)) {
        // No filename yet — return items to queue rather than losing them.
        {
